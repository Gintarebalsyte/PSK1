...
@ApplicationScoped
public class ISBNCodeGenerator implements Serializable {

    @Futureable
    public Future<String> generateISBNCode() {
        try {
            Thread.sleep(3000);
        } catch (InterruptedException e) {
        }

        final String generatedISBNCode = this.makeISBN();
        return new AsyncResult<>(generatedISBNCode);
    }
